    ...
    >
      <v-row align="center" justify="center">
        <v-col class="text-center" cols="12">
          ...
          <h2 class="subheading">
            {{ museumData.country }} {{ museumData.city }}
          </h2>
        </v-col>
      </v-row>
      <v-fab-transition>
  ...
  methods: {
    collectMuseumData: function() {
      api.museum.getMuseumData(this.id).then(response => {
        ...
        api.artifact.getListArtifactOfMuseum(this.id).then(response => {
    ...
    checkIfFavorite: function() {
      api.star.getStarByUserAndMuseum(this.userId, this.id).then(response => {
        if (
